...
def mcnemar(y_true, pred_a, pred_b):
    ...
    df = 1
    pval = stats.chi2.sf(stat, df) # sf is survival function (i.e. 1 - cdf)
    return stat, pval
...
def get_vocab(X, n_words=None, mincount=1):
    ...
    if mincount > 1:
        wc = {(w, c) for w, c in wc if c >= mincount} # keep only words that show up >= mincount
...
def create_pretrained_embedding(
    ...
    """
    dim = len(next(iter(lookup.values()))) # this is the number of features in the lookup dictionary
    embedding = np.array([lookup.get(w, randvec(dim)) for w in vocab]) # dictionary.get method being used here: if word not in dict then use random feature vector
    for tok in required_tokens:
        if tok not in vocab: # add any of the 3 required tokens that aren't included in the vocab
            vocab.append(tok)
